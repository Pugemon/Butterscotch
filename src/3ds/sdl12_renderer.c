...
static void fastDraw_Sprite(SDL_Surface* src, SDL_Rect* sr, SDL_Surface* dst,
    ...
    uint8_t alpha_thresh = (uint8_t)(127.0f / global_alpha);
    ...
    if (is_1to1 && !flipX && !flipY) {
        ...
        for (int y = 0; y < draw_h; y++) {
            uint32_t* src_ptr = (uint32_t*)src->pixels + (sy_start + y) * src_pitch + sx_start;
            uint16_t* dst_ptr = (uint16_t*)dst->pixels + (dy1 + y) * dst_pitch + dx1;
            for (int x = 0; x < draw_w; x++) {
                uint32_t p = *src_ptr++;
                if (((p >> 24) & 0xFF) >= alpha_thresh) {
                    if (use_tint) {
                        uint8_t r = (((p & 0xFF) * tr) >> 8);
                        uint8_t g = ((((p >> 8) & 0xFF) * tg) >> 8);
                        uint8_t b = ((((p >> 16) & 0xFF) * tb) >> 8);
                        *dst_ptr = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3);
                    } else {
                        *dst_ptr = ((p & 0xF8) << 8) | ((p >> 5) & 0x07E0) | ((p >> 19) & 0x1F);
                    }
                }
                dst_ptr++;
            }
        ...
        uint32_t cur_sy_fp = (dy1 - dstY) * ratio_y_fp;
        ...
        for (int y = 0; y < draw_h; y++) {
            ...
            uint32_t* src_row = (uint32_t*)src->pixels + actual_sy * src_pitch;
            uint16_t* dst_ptr = (uint16_t*)dst->pixels + (dy1 + y) * dst_pitch + dx1;
            ...
            for (int x = 0; x < draw_w; x++) {
                ...
                int actual_sx = flipX ? (sr->x + sr->w - 1 - sx) : (sr->x + sx);
                uint32_t p = src_row[actual_sx];
...
static void fastDraw_Text(SDL_Surface* src, SDL_Rect* sr, SDL_Surface* dst,
    ...
    int src_pitch = src->pitch / 4, dst_pitch = dst->pitch / 2;
    bool is_1to1 = (dstW == sr->w && dstH == sr->h);
    bool use_tint = (tint != 0xFFFFFF);
    uint32_t tr = (tint >> 16) & 0xFF, tg = (tint >> 8) & 0xFF, tb = tint & 0xFF;

    uint32_t ratio_x_fp = is_1to1 ? 0 : (sr->w << 16) / dstW;
    uint32_t ratio_y_fp = is_1to1 ? 0 : (sr->h << 16) / dstH;
    uint32_t cur_sy_fp = is_1to1 ? 0 : (dy1 - dstY) * ratio_y_fp;
    uint32_t start_sx_fp = is_1to1 ? 0 : (dx1 - dstX) * ratio_x_fp;

    for (int y = 0; y < draw_h; y++) {
        int sy = is_1to1 ? (sr->y + (dy1 - dstY) + y) : (sr->y + (cur_sy_fp >> 16));
        if (!is_1to1) cur_sy_fp += ratio_y_fp;

        uint32_t* src_row = (uint32_t*)src->pixels + sy * src_pitch;
        uint16_t* dst_ptr = (uint16_t*)dst->pixels + (dy1 + y) * dst_pitch + dx1;
        uint32_t cur_sx_fp = start_sx_fp;

        for (int x = 0; x < draw_w; x++) {
            int sx = is_1to1 ? (sr->x + (dx1 - dstX) + x) : (sr->x + (cur_sx_fp >> 16));
            if (!is_1to1) cur_sx_fp += ratio_x_fp;

            uint32_t p = src_row[sx];
            uint32_t a = (p >> 24) & 0xFF;
            a = (uint32_t)(a * global_alpha);

            if (a > 0) {
                uint32_t fr = use_tint ? (((p & 0xFF) * tr) >> 8) : (p & 0xFF);
                uint32_t fg = use_tint ? ((((p >> 8) & 0xFF) * tg) >> 8) : ((p >> 8) & 0xFF);
                uint32_t fb = use_tint ? ((((p >> 16) & 0xFF) * tb) >> 8) : ((p >> 16) & 0xFF);

                if (a > 250) {
                    *dst_ptr = ((fr >> 3) << 11) | ((fg >> 2) << 5) | (fb >> 3);
                } else {
                    uint16_t bg = *dst_ptr;
                    uint32_t br = (bg >> 11) << 3;
                    uint32_t bg_g = ((bg >> 5) & 0x3F) << 2;
                    uint32_t bb = (bg & 0x1F) << 3;
                    uint32_t inv_a = 255 - a;
                    uint32_t out_r = (fr * a + br * inv_a) >> 8;
                    uint32_t out_g = (fg * a + bg_g * inv_a) >> 8;
                    uint32_t out_b = (fb * a + bb * inv_a) >> 8;
                    *dst_ptr = ((out_r >> 3) << 11) | ((out_g >> 2) << 5) | (out_b >> 3);
                }
            }
            dst_ptr++;
        }
...
static void ensureTextureLoaded(SDLRenderer* sdl, int16_t pageId) {
    ...
    SDL_Surface* tmp = SDL_CreateRGBSurfaceFrom(px, w, h, 32, w * 4, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
    SDL_Surface* opt = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 32, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);

    if (tmp && opt) {
        SDL_SetAlpha(tmp, 0, 0);
        SDL_BlitSurface(tmp, NULL, opt, NULL);
        sdl->sdlSurfaces[pageId] = opt;
    }
...
static void sdlBeginFrame(Renderer* r, int32_t gw, int32_t gh, int32_t ww, int32_t wh) {
    ...
    if (sdl->fboWidth != sdl->gameW || sdl->fboHeight != sdl->gameH || !sdl->fboSurface) {
        if (sdl->fboSurface) SDL_FreeSurface(sdl->fboSurface);
        SDL_Surface* t = SDL_CreateRGBSurface(SDL_SWSURFACE, sdl->gameW, sdl->gameH, 16, RMASK16, GMASK16, BMASK16, AMASK16);
        if(t) {
            SDL_Surface* opt = SDL_DisplayFormat(t);
            sdl->fboSurface = opt ? opt : t;
            if (opt) SDL_FreeSurface(t);
        }
...
static int32_t sdlCreateSpriteFromSurface(Renderer* r, int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    SDL_Surface* ns = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 32,
                                           0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
    if (!ns) return -1;

    SDL_Rect sr = {x, y, w, h};
    SDL_SetAlpha(ns, 0, 255);
    SDL_BlitSurface(sdl->fboSurface, &sr, ns, NULL);
    SDL_SetAlpha(ns, SDL_SRCALPHA, 255);
