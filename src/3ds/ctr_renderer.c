...
static void ctr_free_surface(Renderer *ren, int32_t surfaceId) {
    ...
    surf->used = false;
    // Storage оставляем для реиспользования
}
...
static int32_t ctr_create_surf_ex(Renderer *ren, int32_t surfaceId,
    ...
    // Создаём целевую linear-память текстуру RGBA8
    ...
    // Выгружаем срез из VRAM-tex в линейный буфер, кропаем, переносим обратно тайлами.
    {
