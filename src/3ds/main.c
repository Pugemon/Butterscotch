...
#include <SDL/SDL.h> // ИЗМЕНЕНО ДЛЯ SDL 1.2
#include <getopt.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char* argv[]) {
    ...
    char* my_argv[] = {
        "butterscotch.3dsx",
        "sdmc:/3ds/butterscotch/undertale/data.win"
        //"sdmc:/3ds/butterscotch/Katana_ZERO/data.win"
    };
    int my_argc = 2;
    ...
    N3dsFileSystem* fileSystem = N3dsFileSystem_create(args.dataWinPath);
    ...
    while (!runner->shouldExit && aptMainLoop()) {
        ...
        struct { uint32_t mask; int32_t gmlKey; } btnMap[] = {
            { KEY_DUP | KEY_CPAD_UP, VK_UP },
            { KEY_DDOWN | KEY_CPAD_DOWN, VK_DOWN },
            { KEY_DLEFT | KEY_CPAD_LEFT, VK_LEFT },
            { KEY_DRIGHT | KEY_CPAD_RIGHT, VK_RIGHT },
            { KEY_A, 'Z' },
            { KEY_B, 'X' },
            { KEY_X | KEY_Y, 'C' },
            { KEY_START, VK_ENTER },
        ...
        if (!InputRecording_isPlaybackActive(globalInputRecording)) {
            static bool prevBtnState[256] = { false };
            static bool prevTouchState = false;

            for (int i = 0; i < sizeof(btnMap) / sizeof(btnMap[0]); i++) {
                int32_t gmlKey = btnMap[i].gmlKey;
                bool isHeld = (kHeld & btnMap[i].mask) != 0;

                if (isHeld && !prevBtnState[gmlKey]) {
                    RunnerKeyboard_onKeyDown(runner->keyboard, gmlKey);
                } else if (!isHeld && prevBtnState[gmlKey]) {
                    RunnerKeyboard_onKeyUp(runner->keyboard, gmlKey);
                }
                prevBtnState[gmlKey] = isHeld;
            }

            bool isTouching = (kHeld & KEY_TOUCH) != 0;

            if (isTouching && !prevTouchState) {
                RunnerKeyboard_onMouseDown(runner->keyboard, 1);
            } else if (!isTouching && prevTouchState) {
                RunnerKeyboard_onMouseUp(runner->keyboard, 1);
            }
            prevTouchState = isTouching;

            if (isTouching) {
                touchPosition touch;
                hidTouchRead(&touch);

                runner->keyboard->mouseX = (float)touch.px * ((float)gen8->defaultWindowWidth / 320.0f);
                runner->keyboard->mouseY = (float)touch.py * ((float)gen8->defaultWindowHeight / 240.0f);
            }
        }
#endif
        SDL_Event event;
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (!InputRecording_isPlaybackActive(globalInputRecording)) {
                    int32_t btn = -1;
                    if      (event.button.button == SDL_BUTTON_LEFT)   btn = 1; // mb_left
                    else if (event.button.button == SDL_BUTTON_RIGHT)  btn = 2; // mb_right
                    else if (event.button.button == SDL_BUTTON_MIDDLE) btn = 3; // mb_middle
                    if (btn >= 1) {
        ...
        if (shouldScreenshot) {
            captureScreenshot(renderer, args.screenshotPattern, runner->frameCount, (int) gen8->defaultWindowWidth, (int) gen8->defaultWindowHeight);
        }
        ...
        if (!args.headless && runner->currentRoom->speed > 0) {
            ...
            if (remaining > 0.0) {
                SDL_Delay((uint32_t)(remaining * 1000.0));
            }
    ...
    Runner_free(runner);
    N3dsFileSystem_destroy(fileSystem);
    VM_free(vm);
    DataWin_free(dataWin);
