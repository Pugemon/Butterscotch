...
typedef struct RunnerKeyboardState {
    ...
    #define GML_MOUSE_BUTTON_COUNT 4
    bool mouseDown[GML_MOUSE_BUTTON_COUNT];
    bool mousePressed[GML_MOUSE_BUTTON_COUNT];
    bool mouseReleased[GML_MOUSE_BUTTON_COUNT];

    // Mouse position in GML world-space (updated by platform layer each frame)
    float mouseX;
    float mouseY;
} RunnerKeyboardState;
