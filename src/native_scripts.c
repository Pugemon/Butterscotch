...
static int32_t findScriptCodeId(VMContext *ctx, const char *name) {
    ...
    if (0 > idx) {
        fprintf(stderr, "NativeScripts: WARNING - script '%s' not found in codeIndexByName\n", name);
        return -1;
    }
...
static void selfArraySet(Instance *inst, int32_t varId, int32_t index, RValue val) {
    ...
    if (slot == NULL) return;
    RValue_free(slot);
    if (val.type == RVALUE_STRING && !val.ownsReference && val.string != NULL) {
    ...
    for (int32_t n = 1; stringpos >= n; n++) {
        ...
        if (ch == '^' && nativeStringCharAtBuf(originalstring, origStrLen, n + 1) != '0') {
            ...
            switch (ch) {
                case 'R': mycolor = 255;
                    break;
                case 'G': mycolor = 65280;
                    break;
                case 'W': mycolor = 16777215;
                    break;
                case 'Y': mycolor = 65535;
                    break;
                case 'X': mycolor = 0;
                    break;
                case 'B': mycolor = 16711680;
                    break;
                case 'O': mycolor = 4235519;
                    break;
                case 'L': mycolor = 16629774;
                    break;
                case 'P': mycolor = 16711935;
                    break;
                case 'p': mycolor = 13941759;
                    break;
            ...
            if (handled) {
                ...
                if (sprite != -4) {
                    ...
                    if (vtext) {
                        int32_t sprWidth = (sprite >= 0 && dw->sprt.count > (uint32_t) sprite)
                                               ? (int32_t) dw->sprt.sprites[sprite].width
                                               : 0;
...
#pragma GCC diagnostic pop
    ...
    {
        u64 _wrT1;
        sceRtcGetCurrentTick(&_wrT1);
        g_writerTimeUs += (uint32_t) (_wrT1 - _wrT0);
        g_writerCalls++;
    }
#endif
}
...
static void initChasefire2Cache(VMContext *ctx, DataWin *dw) {
    ...
    chasefire2Cache.ready = (chasefire2Cache.goofId >= 0 && chasefire2Cache.heartObj >= 0 && chasefire2Cache.
                             idealborderId >= 0);
}

static Instance *findInstanceByObject(Runner *runner, int32_t objIdx) {
    if (runner == NULL || runner->instancesByObject == NULL || objIdx < 0 ||
        (uint32_t) objIdx >= runner->dataWin->objt.count)
        return NULL;
...
static RValue native_scr_gettext(VMContext *ctx, RValue *args, int32_t argCount) {
    ...
    if (textVal.type == RVALUE_STRING && textVal.string) text = textVal.string;
        ...
        char *ts = RValue_toString(textVal);
        RValue_free(&textVal);
        textVal = RValue_makeOwnedString(ts);
        text = ts;
    }
...
static void native_readable_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (mi == 1) {
        ...
        repeat((int32_t)arrlen(runner->instances), i) {
            if (runner->instances[i]->active && (int32_t) runner->instances[i]->instanceId == dlgId) {
                exists = true;
                break;
            }
        }
...
static struct {
    int32_t yarl, plot, flag;
    bool ready;
} spikesCache = {.ready = false};
...
static void native_torbody_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    int32_t count = (int32_t) arrlen(runner->instances);
    for (int32_t i = 0; i < count; i++) {
        if (runner->instances[i]->active && runner->instances[i]->objectIndex == 764) {
            exists = true;
            break;
        }
    }
...
static void native_finalbarrier_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    {
        int64_t k = ((int64_t) barrierCache.c_var << 32) | 0u;
        ptrdiff_t i = hmgeti(inst->selfArrayMap, k);
        if (i >= 0) c0 = RValue_toReal(inst->selfArrayMap[i].value);
    }
    {
        int64_t k = ((int64_t) barrierCache.c_var << 32) | 1u;
        ptrdiff_t i = hmgeti(inst->selfArrayMap, k);
        if (i >= 0) c1 = RValue_toReal(inst->selfArrayMap[i].value);
    }
    {
        int64_t k = ((int64_t) barrierCache.c_var << 32) | 2u;
        ptrdiff_t i = hmgeti(inst->selfArrayMap, k);
        if (i >= 0) c2 = RValue_toReal(inst->selfArrayMap[i].value);
    }
...
static struct {
    int32_t goalsize, size_var, siner, gg, rr, vv;
    bool ready;
} orangeCache = {.ready = false};
...
// =====================================================================
static struct {
    int32_t pg_siner, pg_amt, pg_fade;
    bool ready;
} purplegradCache = {.ready = false};
...
static void native_itemswapper_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (kbPressed) {
        ...
        if (RValue_toReal(kbPressed(ctx, &k, 1)) > 0 && column != 1) {
            column = 1;
            c1y = c0y;
        }
        ...
        if (pressed0 && buffer > 6) {
            if (column == 0 && swapperCache.scrStorageget >= 0) {
                ...
                if (noroom == 0 && swapperCache.scrStorageshift >= 0) {
                    RValue shift[3] = {
                        RValue_makeReal((GMLReal) c1y), RValue_makeReal(0), RValue_makeReal((GMLReal) boxno)
                    };
...
static void native_clawbullet_white_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (inst->hspeed < 0 && inst->x < (borderPosCache.lbx - border12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->hspeed > 0 && inst->x > (borderPosCache.rbx + border12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->vspeed < 0 && inst->y < (borderPosCache.border2 - border12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->vspeed > 0 && inst->y > (borderPosCache.border3 + border12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
...
static void native_4sidebullet_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    updateBorderPosCache(ctx, runner);
    float b12 = 12.0f;
    if (inst->hspeed < 0 && inst->x < (borderPosCache.lbx - b12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->hspeed > 0 && inst->x > (borderPosCache.rbx + b12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->vspeed < 0 && inst->y < (borderPosCache.border2 - b12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->vspeed > 0 && inst->y > (borderPosCache.border3 + b12)) {
        Runner_destroyInstance(runner, inst);
        return;
    }
...
static struct {
    int32_t objDrakehead;
    bool ready;
} drakebodyCache = {.ready = false};
...
static void native_drakebody_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *head = (drakebodyCache.objDrakehead >= 0)
                         ? findInstanceByObject(runner, drakebodyCache.objDrakehead)
                         : NULL;
...
static struct {
    int32_t r, ang, rspeed, angspeed, centerx, centery;
    bool ready;
} cfireCache = {.ready = false};
...
static struct {
    int32_t dr, turntimer;
    bool ready;
} firestormCache = {.ready = false};
...
static void native_sidedfire_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Sprite *spr = (inst->spriteIndex >= 0 && (uint32_t) inst->spriteIndex < dw->sprt.count)
                      ? &dw->sprt.sprites[inst->spriteIndex]
                      : NULL;
...
static void native_asgorespear_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Renderer_drawSpriteExt(r, 643, imgIdx, inst->x - (float) xhand1, inst->y - (float) yhand1, 2.0f, 2.0f, fAngle,
                           0xFFFFFF, 1.0f);
}
...
static void native_battlecontroller_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (bgColor != 0xFFFFFF && drawbinfo == 1) {
        ...
        if (flag271 == 0) {
            ...
            if (flag509 == 1) hpwrite = "00.001";
                ...
                snprintf(hpBuf, sizeof(hpBuf), "0%d", (int32_t) hp);
                hpwrite = hpBuf;
            } else {
                snprintf(hpBuf, sizeof(hpBuf), "%d", (int32_t) hp);
                hpwrite = hpBuf;
            }
                ...
                if (isJa) {
                    ...
                    for (int32_t j = 0; name[j];) {
                        ...
                        if (b < 0x80) {
                            ...
                            ch = ((b & 0x1F) << 6) | (name[j + 1] & 0x3F);
                            j += 2;
                        } else if (b < 0xF0) {
                            ch = ((b & 0x0F) << 12) | ((name[j + 1] & 0x3F) << 6) | (name[j + 2] & 0x3F);
                            j += 3;
                        } else {
                            ch = 0;
                            j++;
    ...
    if (isJa && (int32_t) bmenuno >= 3 && bmenuno < 4.0 && (int32_t) myfight == 0 && (int32_t) mnfight == 0) {
        ...
        if (num_items > 3) {
            float bx = (bcCache.idealborder >= 0)
                           ? (float) getGlobalArray(ctx, bcCache.idealborder, 1) - 30.0f
                           : 270.0f;
...
static void native_time_Draw77(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (cachedXofs < 0) {
        cachedXofs = findGlobalVarId(ctx, "window_xofs");
        cachedYofs = findGlobalVarId(ctx, "window_yofs");
    }
    if (cachedXofs >= 0) {
        RValue_free(&ctx->globalVars[cachedXofs]);
        ctx->globalVars[cachedXofs] = RValue_makeReal(0.0);
    }
    if (cachedYofs >= 0) {
        RValue_free(&ctx->globalVars[cachedYofs]);
        ctx->globalVars[cachedYofs] = RValue_makeReal(0.0);
    }
}
...
static void initMaincharaCache(VMContext *ctx, DataWin *dw) {
    ...
    for (uint32_t ci = 0; ci < dw->code.count; ci++) {
        if (strcmp(dw->code.entries[ci].name, "gml_Object_obj_mainchara_Step_0") == 0) {
            maincharaCache.mainCharaCodeId = (int32_t) ci;
            break;
        }
    }
...
static void refreshXoxoFrameCache(Runner *runner) {
    ...
    xoxoCache.xoxoctrlInst = (xoxoCache.objXoxoctrl >= 0)
                                 ? findInstanceByObject(runner, xoxoCache.objXoxoctrl)
                                 : NULL;
}
...
static void native_specialtile_Alarm0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (kingrando == 1) {
        ...
        if (specialtileCache.frame != frame) {
            ...
            specialtileCache.papyrus4 = (specialtileCache.objPapyrus4 >= 0)
                                            ? findInstanceByObject(runner, specialtileCache.objPapyrus4)
                                            : NULL;
        }
    ...
    switch (gg) {
        case 0: mygrey = 16711680;
            break;
        case 1: mygrey = 65535;
            break;
        case 2: mygrey = 65280;
            break;
        case 3: mygrey = 8388736;
            break;
        case 4: mygrey = 4235519;
            break;
        case 5: mygrey = 255;
            break;
        case 6: mygrey = (255) | (100 << 8) | (100 << 16);
            break;
        default: break;
    }
...
static void initDialoguerCache(VMContext *ctx, DataWin *dw) {
    ...
    for (uint32_t ci = 0; ci < dw->code.count; ci++) {
        if (strcmp(dw->code.entries[ci].name, "gml_Object_obj_dialoguer_Step_0") == 0) {
            dialoguerCache.dialoguerCodeStepId = (int32_t) ci;
            break;
        }
    }
...
static Instance *findInstanceByInstId(Runner *runner, uint32_t id) {
    ptrdiff_t idx = hmgeti(runner->instancesById, id);
    if (idx < 0) return NULL;
    return runner->instancesById[idx].value;
}
...
static void native_dialoguer_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (dialoguerCache.objFace >= 0 && runner->instancesByObject != NULL &&
        ...
        if (face) {
            if (side == 0) { if (face->y > (vy + 80.0f)) face->y -= 155.0f; } else {
                if (face->y < (vy + 80.0f)) face->y += 155.0f;
            }
        }
    }
...
static void initOvrctrlCache(VMContext *ctx, DataWin *dw) {
    ...
    for (uint32_t ci = 0; ci < dw->code.count; ci++) {
        if (strcmp(dw->code.entries[ci].name, "gml_Object_obj_overworldcontroller_Draw_0") == 0) {
            ovrctrlCache.codeId = (int32_t) ci;
            break;
        }
    }
...
static inline void ovrctrl_getTextArg2(VMContext *ctx, const char *key, const char *a1, const char *a2, char *out,
                                       size_t outSize) {
    BuiltinFunc gt = VMBuiltins_find("scr_gettext");
    if (!gt) {
        out[0] = '\0';
...
static void native_overworldctrl_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (interact == 5) {
        if (menuno == 5 && ovrctrl_controlPressed(ctx, 0) &&
            (int32_t) getGlobalArray(ctx, ovrctrlCache.gMenucoord, 5) == 2)
            fallbackVM = true;
    ...
    if (interact == 5) {
        ...
        if (menuno != 4) {
            r->drawColor = 0xFFFFFF;
            r->vtable->drawRectangle(r, 16.0f + xx, 16.0f + moveyy, 86.0f + xx, 70.0f + moveyy, 0xFFFFFF, r->drawAlpha,
                                     false);
            r->vtable->drawRectangle(r, 16.0f + xx, 74.0f + yy, 86.0f + xx, 147.0f + yy, 0xFFFFFF, r->drawAlpha, false);

            if (menuno == 1 || menuno == 5 || menuno == 6) {
                r->vtable->drawRectangle(r, 94.0f + xx, 16.0f + yy, 266.0f + xx, 196.0f + yy, 0xFFFFFF, r->drawAlpha,
                                         false);
            } else if (menuno == 2) {
                float xend = 266.0f;
                if (isJa) xend += 9.0f;
                r->vtable->drawRectangle(r, 94.0f + xx, 16.0f + yy, xend + xx, 224.0f + yy, 0xFFFFFF, r->drawAlpha,
                                         false);
            } else if (menuno == 3) {
                r->vtable->drawRectangle(r, 94.0f + xx, 16.0f + yy, 266.0f + xx, 150.0f + yy, 0xFFFFFF, r->drawAlpha,
                                         false);
            } else if (menuno == 7) {
                r->vtable->drawRectangle(r, 94.0f + xx, 16.0f + yy, 266.0f + xx, 216.0f + yy, 0xFFFFFF, r->drawAlpha,
                                         false);
            }
            ...
            snprintf(buf, sizeof(buf), "%d/%d", (int32_t) globalReal(ctx, ovrctrlCache.gHp),
                     (int32_t) globalReal(ctx, ovrctrlCache.gMaxhp));
        ...
        if (menuno == 2) {
            ...
            char out[256];
            ovrctrl_getTextArg(ctx, "stat_menu_lv", tmp, out, sizeof(out));
            ...
            static const int32_t xpThresh[] = {
                10, 30, 70, 120, 200, 300, 500, 800, 1200, 1700, 2500, 3500, 5000, 7000, 10000, 15000, 25000, 50000,
                99999
            };
        ...
        if (menuno == 4) {
            ...
            if (iniReadStr) {
                ...
                RValue args[3] = {
                    RValue_makeString("General"), RValue_makeString("Name"),
            ...
            if (iniReadReal) {
                RValue a[3];
                a[0] = RValue_makeString("General");
                a[1] = RValue_makeString("Love");
                a[2] = RValue_makeReal(0);
                RValue rr = iniReadReal(ctx, a, 3);
                love = RValue_toReal(rr);
                RValue_free(&rr);
                a[1] = RValue_makeString("Time");
                a[2] = RValue_makeReal(1);
                rr = iniReadReal(ctx, a, 3);
                time_v = RValue_toReal(rr);
                RValue_free(&rr);
                a[1] = RValue_makeString("Kills");
                a[2] = RValue_makeReal(0);
                rr = iniReadReal(ctx, a, 3);
                killsV = RValue_toReal(rr);
                RValue_free(&rr);
                (void) killsV;
                a[1] = RValue_makeString("Room");
                a[2] = RValue_makeReal(0);
                rr = iniReadReal(ctx, a, 3);
                roomeV = RValue_toReal(rr);
                RValue_free(&rr);
            }
            ...
            r->drawColor = 0xFFFFFF;
            r->vtable->drawRectangle(r, 54.0f + xx, 49.0f + yy, 265.0f + xx, 135.0f + yy, 0xFFFFFF, r->drawAlpha,
                                     false);
            r->drawColor = 0;
            ...
            if (isJa && scrDrawtextCenteredId >= 0) {
                RValue args[3] = {
                    RValue_makeReal(x_center), RValue_makeReal(80.0 + yy),
            ...
            if (kbPressed4) {
                ...
                bool left = RValue_toReal(rr37) != 0;
                RValue_free(&rr37);
                RValue k39 = RValue_makeReal(39);
                RValue rr39 = kbPressed4(ctx, &k39, 1);
                bool right = RValue_toReal(rr39) != 0;
                RValue_free(&rr39);
                if ((left || right) && mc4 < 2) {
                    ...
                    if (kbClear) {
                        RValue k = RValue_makeReal(37);
                        RValue rr = kbClear(ctx, &k, 1);
                        RValue_free(&rr);
                        k = RValue_makeReal(39);
                        rr = kbClear(ctx, &k, 1);
                        RValue_free(&rr);
        ...
        if (kbPressed) {
            ...
            bool up = RValue_toReal(rr) != 0;
            RValue_free(&rr);
            RValue k40 = RValue_makeReal(40);
            rr = kbPressed(ctx, &k40, 1);
            bool down = RValue_toReal(rr) != 0;
            RValue_free(&rr);
            RValue k37 = RValue_makeReal(37);
            rr = kbPressed(ctx, &k37, 1);
            bool left = RValue_toReal(rr) != 0;
            RValue_free(&rr);
            RValue k39 = RValue_makeReal(39);
            rr = kbPressed(ctx, &k39, 1);
            bool right = RValue_toReal(rr) != 0;
            RValue_free(&rr);
...
static void initBouncerightCache(VMContext *ctx, DataWin *dw) {
    ...
    for (uint32_t ci = 0; ci < dw->code.count; ci++) {
        if (strcmp(dw->code.entries[ci].name, "gml_Object_obj_bounceright_Step_0") == 0) {
            bouncerightCache.codeId = (int32_t) ci;
            break;
        }
    }
...
static void native_spiderstrand_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    uint32_t iFull = (uint32_t) (int32_t) s;
    if (iFull > 255) iFull = 255;
    uint32_t iOful = (uint32_t) (int32_t) c;
    if (iOful > 255) iOful = 255;
...
static void native_trueLavawaver_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    int32_t spriteW = (spriteIdx < ctx->dataWin->sprt.count)
                          ? (int32_t) ctx->dataWin->sprt.sprites[spriteIdx].width
                          : 0;
    int32_t subimg = (int32_t) inst->imageIndex;
...
static void native_plusbomb_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    float x1, x2;
    if (side == 1) {
        x1 = inst->x + 2.0f + spriteW + 1.0f;
        x2 = inst->x + 22.0f + spriteW + 1.0f;
    } else {
        x1 = inst->x + 2.0f - spriteW - 1.0f;
        x2 = inst->x + 22.0f - spriteW - 1.0f;
    }
...
static void leglineDrawShared(VMContext *ctx, Runner *runner, Instance *inst, bool right) {
    ...
    if (con == 1.0 && myspeed > 0.0) {
        ...
        if (eoi == 0) color = 0x0000FFu;
            ...
            if (sp) {
                RValue a = RValue_makeReal(119.0);
                Instance *saved = (Instance *) ctx->currentInstance;
                ctx->currentInstance = inst;
                RValue rr = sp(ctx, &a, 1);
                ctx->currentInstance = saved;
                RValue_free(&rr);
            }
        ...
        if (!right) {
            x1 = (float) (ibL + 6.0);
            x2 = (float) (ibL + attackLen - 6.0);
        } else {
            x1 = (float) (ibR - 6.0);
            x2 = (float) (ibR - attackLen + 6.0);
        }
        r->vtable->drawRectangle(r, x1, (float) (ibT + 6.0), x2, (float) (ibB - 4.0),
                                 color, 1.0f, true);
        if (!right) {
            x1 = (float) (ibL + 7.0);
            x2 = (float) (ibL + attackLen - 5.0);
        } else {
            x1 = (float) (ibR - 7.0);
            x2 = (float) (ibR - attackLen + 5.0);
        }
    ...
    if (con >= 3.0) {
        ...
        for (int32_t i = 0; i < 5; i++) {
            ...
            if (collRect) {
                ...
                if (!right) {
                    cx1 = (float) ibL;
                    cx2 = (float) (ibL + myx - 30.0);
                } else {
                    cx1 = (float) ibR;
                    cx2 = (float) (ibR - myx + 30.0);
                }
                RValue args[7] = {
            ...
            if (collLine) {
                float lx1, lx2;
                if (!right) {
                    lx1 = (float) (ibL + myx - 30.0);
                    lx2 = (float) (ibL + myx - 8.0);
                } else {
                    lx1 = (float) (ibR - myx + 30.0);
                    lx2 = (float) (ibR - myx + 8.0);
                }
                RValue args[7] = {
...
static void native_mettbBody_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (bodyopen == 1) { if (bodyimg < 5.0) bodyimg += 0.25; } else if (bodyimg > 0.0) bodyimg -= 0.25;
    ...
    if (fadewhite == 1) {
        ...
        if (whiteval == 10.0f) {
            ...
            if (fv == 1.0) {
                BuiltinFunc sp = VMBuiltins_find("snd_play");
                if (sp) {
                    RValue aa = RValue_makeReal(92.0);
                    Instance *saved = (Instance *) ctx->currentInstance;
                    ctx->currentInstance = inst;
                    RValue rr = sp(ctx, &aa, 1);
                    ctx->currentInstance = saved;
                    RValue_free(&rr);
                }
            }
...
static inline void ratingsDrawText(Renderer *r, const char *text, float x, float y,
    ...
    uint32_t savedC = r->drawColor;
    float savedA = r->drawAlpha;
    r->drawColor = color;
    r->drawAlpha = alpha;
    ...
    *outSiner = siner;
    ...
    if (flash == 0) {
        ...
        if (r && r->vtable->drawEllipse) {
            ...
            uint32_t ecol;
            if (variant == 1) {
                bx1 = inst->x + (float) op + 70;
                by1 = inst->y + 50;
                bx2 = inst->x + (float) op - 30;
                by2 = inst->y - 40;
                ecol = 0x40A0FFu;
            } else if (variant == 2) {
                bx1 = inst->x - 105 + (float) op;
                by1 = inst->y + 40;
                bx2 = inst->x + 85 + (float) op;
                by2 = inst->y - 40;
                ecol = 0xFF00FFu;
            } else {
                bx1 = inst->x - 45 + (float) op;
                by1 = inst->y + 40;
                bx2 = inst->x + 45 + (float) op;
                by2 = inst->y - 40;
                ecol = 0x00FFFFu;
            }
            if (bx1 > bx2) {
...
static void native_floweyMouth_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (mode == 0) {
        if (rotbonus > 0.0) rotbonus -= 5.0;
        else rotbonus = 0.0;
        if (xbonus > 0.0) xbonus -= 2.0;
        else xbonus = 0.0;
        if (ybonus < 0.0) ybonus += 2.0;
        else ybonus = 0.0;
        ybonus = 0.0;
    }
    if (mode == 1) {
...
static void floweyLeftEyeSetBullet(VMContext *ctx, Instance *bullet, Instance *heart,
    ...
    bullet->friction = frict;
    float dirBump = (wimpy == 0)
                        ? (18.0f - (float) durara * 18.0f)
...
static void native_gigavinePrev_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (nowtime > maxtime) {
        ...
        int32_t memorymode = (gigavinePrevCache.memorymode >= 0)
                                 ? selfInt(inst, gigavinePrevCache.memorymode)
                                 : 0;
        if (memorymode == 0) {
            Runner_destroyInstance(runner, inst);
        } else {
...
static void native_floweyTv_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    switch (tvmode) {
        ...
        case 1: {
            ...
            if (sprAlpha < 0.0f) sprAlpha = 0.0f;
            if (sprAlpha > 1.0f) sprAlpha = 1.0f;
        ...
        case 20: {
            r->vtable->drawRectangle(r, fx + 20.0f, fy + 50.0f, fx + 160.0f, fy + 140.0f,
                                     0xFFFFFFu, 1.0f, false);
            siner += 1.0;
            Renderer_drawSprite(r, inst->spriteIndex, imgIdx, fx,
                                fy + (float) (GMLReal_sin((siner * M_PI) / 2.0) * 2.0));
...
static struct {
    int32_t siner, frozen, desperate;
    bool ready;
} fleshfaceCache = {.ready = false};
...
static void native_wordbullet_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (inst->hspeed > 0.0f && inst->x > 405.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->hspeed < 0.0f && inst->x < 120.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
...
static struct {
    int32_t conVarIds[ACT_MAX_CON_VARIDS];
    int32_t conCount;
    int32_t type;
    bool ready;
...
static void native_fogmaker_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (sFlag == 0) {
        Instance *mainchara = (fogmakerCache.objMainchara >= 0)
                                  ? findInstanceByObject(runner, fogmakerCache.objMainchara)
                                  : NULL;
...
static void native_topbone_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (inst->x < (ibL - 10.0f) && inst->hspeed < 0.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->x > (ibR + 10.0f) && inst->hspeed > 0.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
}
...
static void native_coolbus_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (inst->x < (ibL - 100.0f) && inst->hspeed < 0.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
    if (inst->x > (ibR + 100.0f) && inst->hspeed > 0.0f) {
        Runner_destroyInstance(runner, inst);
        return;
    }
...
static void native_glowstone_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *dp = (glowstoneCache.objDarknesspuzzle >= 0)
                       ? findInstanceByObject(runner, glowstoneCache.objDarknesspuzzle)
                       : NULL;
...
static void native_darknesspuzzle_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *mc = (darknesspuzzleCache.objMainchara >= 0)
                       ? findInstanceByObject(runner, darknesspuzzleCache.objMainchara)
                       : NULL;
    if (mc == NULL) return;
...
static void native_puddle_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *mainchara = (puddleCache.objMainchara >= 0)
                              ? findInstanceByObject(runner, puddleCache.objMainchara)
                              : NULL;
...
static void native_undynespear_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (findInstanceByObject(runner, 1119) != NULL) {
        Instance *actor = (undyneSpearCache.objUndyneaActor >= 0)
                              ? findInstanceByObject(runner, undyneSpearCache.objUndyneaActor)
                              : NULL;
    ...
    if (active == 1) {
        ...
        if (ramt > 0.0) {
            ...
            Instance *mc = (undyneSpearCache.objMainchara >= 0)
                               ? findInstanceByObject(runner, undyneSpearCache.objMainchara)
                               : NULL;
...
static void native_hotlandsign_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    int32_t greenbright = (hotlandsignCache.greenbright >= 0)
                              ? selfInt(inst, hotlandsignCache.greenbright)
                              : 0;
    struct {
        float dx, dy, dw, dh;
        int32_t highlightAt;
...
static void native_temhand_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *heart = (temhandCache.objHeart >= 0)
                          ? findInstanceByObject(runner, temhandCache.objHeart)
                          : NULL;
    if (heart) {
...
static void native_maddumDrawer_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (mode == 2) {
        ...
        for (int32_t i = 0; i < 4; i++) {
            ...
            GMLReal opr = (maddumCache.opartrot >= 0)
                              ? RValue_toReal(selfArrayGet(inst, maddumCache.opartrot, i))
                              : 0.0;
            px -= (px - opx) / 4.0;
            py -= (py - opy) / 4.0;
            pr -= (pr - opr) / 4.0;
...
static void native_roundedge_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (edge == 0.0) {
        edge = 0.1;
        if (roundedgeCache.edge >= 0) Instance_setSelfVar(inst, roundedgeCache.edge, RValue_makeReal(edge));
    }
    if (part == 0.0) {
        part = 1.0;
        Instance_setSelfVar(inst, roundedgeCache.part, RValue_makeReal(part));
    }
...
static void lastbeam_casterStop(VMContext *ctx, GMLReal snd) {
    ...
    if (ass) {
        RValue a[1] = {RValue_makeReal(snd)};
        RValue r = ass(ctx, a, 1);
        RValue_free(&r);
    }
}
    ...
    if (timer > beamtime) {
        ...
        if (bw > 0.0) {
                ...
                if (blcon) {
                    ...
                    if (blconwd) {
                        ...
                        if (lastbeamCache.blconwd >= 0)
                            Instance_setSelfVar(inst, lastbeamCache.blconwd,
                                                RValue_makeReal((GMLReal) blconwd->instanceId));
                    }
                }
            }
            ...
            if (shakenVal == 1) {
                ...
                for (int32_t j = 0; j < 3; j++) {
                    GMLReal mx = (lastbeamCache.menux >= 0)
                                     ? RValue_toReal(selfArrayGet(inst, lastbeamCache.menux, j))
                                     : 0.0;
                    GMLReal my = (lastbeamCache.menuy >= 0)
                                     ? RValue_toReal(selfArrayGet(inst, lastbeamCache.menuy, j))
                                     : 0.0;
                    double r1 = 0.0, r2 = 0.0;
                    if (rnd) {
                        RValue a = RValue_makeReal(4.0);
                        RValue v1 = rnd(ctx, &a, 1);
                        r1 = RValue_toReal(v1);
                        RValue_free(&v1);
                        RValue v2 = rnd(ctx, &a, 1);
                        r2 = RValue_toReal(v2);
                        RValue_free(&v2);
                    }
                    GMLReal btObjId = (lastbeamCache.menu >= 0)
                                          ? RValue_toReal(selfArrayGet(inst, lastbeamCache.menu, j))
                                          : 0.0;
...
#define AF (&afinalBodyCache)
    ...
    Renderer_drawSpriteExt(r, 2449, anim6, inst->x + 42.0f, (inst->y - 52.0f) + (float) (yoff2 * 4.0), 2.0f, 2.0f, 0.0f,
                           blend, alpha);
    Renderer_drawSpriteExt(r, 2449, anim6, inst->x - 44.0f, (inst->y - 52.0f) + (float) (yoff2 * 4.0), -2.0f, 2.0f,
                           0.0f, blend, alpha);
    ...
    if (rnd) {
        ...
        RValue v1 = rnd(ctx, &a, 1);
        double a1 = RValue_toReal(v1);
        RValue_free(&v1);
        RValue v2 = rnd(ctx, &a, 1);
        double a2 = RValue_toReal(v2);
        RValue_free(&v2);
        RValue v3 = rnd(ctx, &a, 1);
        double b1 = RValue_toReal(v3);
        RValue_free(&v3);
        RValue v4 = rnd(ctx, &a, 1);
        double b2 = RValue_toReal(v4);
        RValue_free(&v4);
        rxv = (a1 - a2) * 0.7;
        ryv = (b1 - b2) * 1.5;
    }
    ...
    if (ucon > 0) {
        ...
        if (ucon == 4) {
            ...
            for (int32_t k = 0; k < nIt; k++) {
                Instance *it = runner->instances[k];
                if (it->active && it->objectIndex == 574) {
                    inst574Exists = true;
                    break;
                }
            }
...
static void native_hgBody_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (con < 3.0) {
        Renderer_drawSpriteExt(r, 2510, 0, inst->x, inst->y + (float) (facey / 6.0), 2.0f, 2.0f, 0.0f, 0xFFFFFFu,
                               inst->imageAlpha);
        Renderer_drawSpriteExt(r, 2508, 0, inst->x, inst->y - (float) (facey / 2.0), 2.0f, 2.0f, 0.0f, 0xFFFFFFu,
                               inst->imageAlpha);
        Renderer_drawSpriteExt(r, 2511, 0, inst->x + 88.0f, inst->y + 72.0f + (float) facey, 2.0f,
                               2.0f + (float) facescale, 0.0f, 0xFFFFFFu, inst->imageAlpha);
        Renderer_drawSpriteExt(r, 2509, 0, inst->x + 104.0f, (inst->y + 248.0f) - (float) (facey / 2.0), 2.0f, 2.0f,
                               0.0f, 0xFFFFFFu, inst->imageAlpha);
    }
...
#undef CALL_RND
        ...
        if (c_counter > 180.0) {
            ...
            if (c_counter > 320.0) {
                ...
                for (int32_t k = 0; k < n; k++) {
                    Instance *it = runner->instances[k];
                    if (it->active && it->objectIndex == 570) {
                        has570 = true;
                        break;
                    }
                }
...
static void native_waterpushrock_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (active == 1 && gInteract != 5.0) {
        ...
        for (int32_t i = 0; i < n; i++) {
            Instance *it = runner->instances[i];
            if (it->active && it->objectIndex == waterpushrockCache.obj1138) {
                has1138 = true;
                break;
            }
        }
        if (has1138) {
            Instance *gen = (waterpushrockCache.objWaterpushrockgen >= 0)
                                ? findInstanceByObject(runner, waterpushrockCache.objWaterpushrockgen)
                                : NULL;
            int32_t colliding = (gen && waterpushrockCache.colliding >= 0)
                                    ? (int32_t) RValue_toReal(Instance_getSelfVar(gen, waterpushrockCache.colliding))
                                    : 0;
            if (colliding == 0) {
                ...
                if (bb.valid && waterpushrockCache.objMainchara >= 0) {
                    ...
                    if (hit != NULL) {
                        ...
                        if (waterpushrockCache.objTime >= 0) {
                            ...
                            if (ot) {
                                int32_t up = (waterpushrockCache.upId >= 0)
                                                 ? (int32_t) RValue_toReal(
                                                     Instance_getSelfVar(ot, waterpushrockCache.upId))
                                                 : 0;
                                int32_t down = (waterpushrockCache.downId >= 0)
                                                   ? (int32_t) RValue_toReal(
                                                       Instance_getSelfVar(ot, waterpushrockCache.downId))
                                                   : 0;
                                if (up) hit->y += 3.0f;
...
#define WBOARD_MAX_TILES 64
    ...
    for (int32_t ii = 0; ii < tileCount; ii++) {
        ...
        for (int32_t j = 1; j <= 3; j++) {
            ...
            if (larr[j] != NULL && lx[j - 1] == 1 && larr[j]->using_val == 3) {
                h += 1;
                lx[j] = 1;
            }
            if (rarr[j] != NULL && rx[j - 1] == 1 && rarr[j]->using_val == 3) {
                h += 1;
                rx[j] = 1;
            }
            if (uarr[j] != NULL && ux[j - 1] == 1 && uarr[j]->using_val == 3) {
                v += 1;
                ux[j] = 1;
            }
            if (darr[j] != NULL && dx[j - 1] == 1 && darr[j]->using_val == 3) {
                v += 1;
                dx[j] = 1;
            }
        ...
        if (v >= 3 && me->using_val == 3) {
            ...
            for (int32_t nn = 1; nn <= 3; nn++) {
                if (uarr[nn] != NULL && ux[nn - 1] == 1) {
                    Instance_setSelfVar(uarr[nn]->inst, usingId, RValue_makeReal(5.0));
                    uarr[nn]->using_val = 5;
                }
                if (darr[nn] != NULL && dx[nn - 1] == 1) {
                    Instance_setSelfVar(darr[nn]->inst, usingId, RValue_makeReal(5.0));
                    darr[nn]->using_val = 5;
                }
            }
        }
        if (h >= 3 && me->using_val == 3) {
            ...
            me->using_val = 5;
            for (int32_t nn = 1; nn <= 3; nn++) {
                if (rarr[nn] != NULL && rx[nn - 1] == 1) {
                    Instance_setSelfVar(rarr[nn]->inst, usingId, RValue_makeReal(5.0));
                    rarr[nn]->using_val = 5;
                }
                if (larr[nn] != NULL && lx[nn - 1] == 1) {
                    Instance_setSelfVar(larr[nn]->inst, usingId, RValue_makeReal(5.0));
                    larr[nn]->using_val = 5;
                }
            }
        }
...
static struct {
    int32_t conCandidates[SPEARTILE_MAX_VARIDS];
    int32_t conCount;
    int32_t facerCandidates[SPEARTILE_MAX_VARIDS];
    int32_t facerCount;
    int32_t soundedCandidates[SPEARTILE_MAX_VARIDS];
    int32_t soundedCount;
    int32_t activeCandidates[SPEARTILE_MAX_VARIDS];
    int32_t activeCount;
    int32_t dutyCandidates[SPEARTILE_MAX_VARIDS];
    int32_t dutyCount;
    int32_t spearbud;
    int32_t sound2;
...
static void initSpeartileCache(DataWin *dw) {
    ...
    speartileCache.soundedCount = findAllSelfVarIds(dw, "sounded", speartileCache.soundedCandidates,
...
static bool speartile_collisionPoint(Runner *runner, DataWin *dw, Instance *self, float px, float py,
                                     int32_t targetObj) {
    if (runner == NULL || runner->instancesByObject == NULL || targetObj < 0 ||
        (uint32_t) targetObj >= runner->dataWin->objt.count)
        return false;
...
static void native_speartile_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (con == 0.0) {
        ...
        if (facer == 1) {
            ...
            for (int32_t i = 0; i < n; i++) {
                Instance *it = runner->instances[i];
                if (it->active && it->objectIndex == speartileCache.objObstacle1575) {
                    has1575 = true;
                    break;
                }
            }
            if (has1575) {
                Instance *ot = (speartileCache.objTime >= 0)
                                   ? findInstanceByObject(runner, speartileCache.objTime)
                                   : NULL;
                int32_t leftK = (ot && speartileCache.left_v >= 0)
                                    ? (int32_t) RValue_toReal(Instance_getSelfVar(ot, speartileCache.left_v))
                                    : 0;
                int32_t rightK = (ot && speartileCache.right_v >= 0)
                                     ? (int32_t) RValue_toReal(Instance_getSelfVar(ot, speartileCache.right_v))
                                     : 0;
                int32_t upK = (ot && speartileCache.up_v >= 0)
                                  ? (int32_t) RValue_toReal(Instance_getSelfVar(ot, speartileCache.up_v))
                                  : 0;
                int32_t downK = (ot && speartileCache.down_v >= 0)
                                    ? (int32_t) RValue_toReal(Instance_getSelfVar(ot, speartileCache.down_v))
                                    : 0;

                if (leftK) {
                    inst->x -= 60.0f;
                    for (int32_t r = 0; r < 3; r++)
                        if (speartile_collisionPoint(runner, dw, inst, inst->x + 9.0f, inst->y + 35.0f,
                                                     speartileCache.objSpeartileWall))
                            inst->x += 20.0f;
                }
                if (rightK) {
                    inst->x += 60.0f;
    ...
    if (fabs(con - 2.5) < 1e-6) {
        ...
        if (bud && bud->imageIndex >= 1.5f && active == 1) {
            ...
            if (sounded == 0) {
                ...
                for (int32_t i = 0; i < n; i++) {
                    Instance *it = runner->instances[i];
                    if (it->active && it->objectIndex == speartileCache.objSoundExists) {
                        has1048 = true;
                        break;
                    }
                }
    ...
    if (duty == 1) {
        Instance *mc = (speartileCache.objMainchara >= 0)
                           ? findInstanceByObject(runner, speartileCache.objMainchara)
                           : NULL;
        if (mc) {
            inst->x = mc->x;
            inst->y = mc->y;
...
static void native_jarfly_Step0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    for (int32_t i = 0; i < n; i++) {
        Instance *it = runner->instances[i];
        if (it->active && it->objectIndex == jarflyCache.objFlyjarExists) {
            jar = it;
            break;
        }
    }
...
static void native_mettatonnnWriter_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    const char *lang = (mettatonnnWriterCache.gLanguage >= 0)
                           ? globalString(ctx, mettatonnnWriterCache.gLanguage)
                           : NULL;
    bool isJa = (lang && strcmp(lang, "ja") == 0);
    ...
    if (isJa) {
        ...
        ystart_right = 116.0f;
        limit_right = 20;
        xstart_bottom = rw - 40.0f;
        ystart_bottom = rh - 20.0f;
        limit_bottom = 32;
        xstart_left = 0.0f;
        ystart_left = 0.0f;
    }
...
static inline void sugarshot_collisionImpl(VMContext *ctx, Runner *runner, Instance *inst,
    ...
    if (snd) {
        RValue a = RValue_makeReal(107.0);
        RValue r = snd(ctx, &a, 1);
        RValue_free(&r);
    }
}
...
static struct {
    int32_t onCandidates[METTNEWS_PART_MAX_VARIDS];
    int32_t onCount;
...
static void native_mettnewsPart_Step2(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    if (on == 1) {
        ...
        for (int32_t i = 0; i < n; i++) {
            Instance *it = runner->instances[i];
            if (it->active && it->objectIndex == mettnewsPartCache.obj185) {
                has185 = true;
                break;
            }
        }
        if (!has185) {
            Instance *mc = (mettnewsPartCache.objMainchara >= 0)
                               ? findInstanceByObject(runner, mettnewsPartCache.objMainchara)
                               : NULL;
            if (mc) {
...
static void native_snowfloor_Draw0(VMContext *ctx, Runner *runner, Instance *inst) {
    ...
    Instance *mc = (snowfloorCache.objMainchara >= 0)
                       ? findInstanceByObject(runner, snowfloorCache.objMainchara)
                       : NULL;
    ...
    for (int32_t i = 0; i < 25; i++) {
        ...
        if (moveme > 1.0) {
            if (mcIsMoving) {
                ...
                flakes[i].sxv->real = snowx;
                flakes[i].sxv->type = RVALUE_REAL;
                flakes[i].syv->real = snowy;
                flakes[i].syv->type = RVALUE_REAL;
            }

            moveme -= 1.0;
            flakes[i].mmv->real = moveme;
            flakes[i].mmv->type = RVALUE_REAL;
        } else if (collided) {
            flakes[i].mmv->real = moveme;
            flakes[i].mmv->type = RVALUE_REAL;
        }
