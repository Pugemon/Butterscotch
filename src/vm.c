 ...
 */
static inline bool shouldTraceVariable(StringBooleanEntry* traceMap, const char* scopeName, const char* altScopeName, const char* varName) {
#ifdef VM_ENABLE_TRACING
    ...
    char formatted[256]; // Избегаем VLA (Variable Length Arrays), они медленные на старых ARM
...
static void handleDiv(VMContext* ctx) {
    ...
    if (divisor == 0.0) {
        // GML does NOT crash on divide by zero - it returns 0 and continues.
        // Using abort() here was the most likely cause of Toriel randomly "freezing"
        // (her AI scripts compute direction vectors that can hit dist==0).
        fprintf(stderr, "VM: [%s] DoDiv :: Divide by zero, returning 0 (GML-compatible)\n", ctx->currentCodeName);
        RValue_free(&a);
        RValue_free(&b);
        stackPush(ctx, RValue_makeReal(0.0));
        return;
    }
...
static void handleRem(VMContext* ctx) {
    ...
    if (ib == 0) {
        fprintf(stderr, "VM: [%s] DoRem :: Divide by zero, returning 0 (GML-compatible)\n", ctx->currentCodeName);
        RValue_free(&a);
        RValue_free(&b);
        stackPush(ctx, RValue_makeInt32(0));
        return;
    }
...
static void handleMod(VMContext* ctx) {
    ...
    GMLReal divisor = RValue_toReal(b);
    if (divisor == 0.0) {
        fprintf(stderr, "VM: [%s] DoMod :: Divide by zero, returning 0 (GML-compatible)\n", ctx->currentCodeName);
        RValue_free(&a);
        RValue_free(&b);
        stackPush(ctx, RValue_makeReal(0.0));
        return;
...
static RValue executeLoop(VMContext* ctx) {
    // Таблица диспетчеризации GCC (маппинг опкодов на метки в коде)
    // Размер 256 покрывает все возможные значения байта opcode (0-255)
    static const void* dispatch_table[256] = {
        // По умолчанию все неизвестные опкоды прыгают на ошибку
        [0 ... 255] = &&op_unknown,
        ...
        [OP_BT]      = &&op_bt,
        [OP_BF]      = &&op_bf,
        [OP_PUSHENV] = &&op_pushenv,
        [OP_POPENV]  = &&op_popenv,
        [OP_PUSH]    = &&op_push,
        [OP_PUSHLOC] = &&op_pushloc,
        [OP_PUSHGLB] = &&op_pushglb,
        [OP_PUSHBLTN]= &&op_pushbltn,
        [OP_CALL]    = &&op_call,
        [OP_BREAK]   = &&op_break
    };
    ...
    // Макрос для чтения следующей инструкции и прыжка на нее.
    // Вызывается в конце каждого обработчика.
    #define DISPATCH() do { \
        if (ctx->ip >= ctx->codeEnd) return RValue_makeUndefined(); \
        instrAddr = ctx->ip; \
        instr = BinaryUtils_readUint32(ctx->bytecodeBase + ctx->ip); \
        ctx->ip += 4; \
        extraData = ctx->bytecodeBase + ctx->ip; \
        if (instrHasExtraData(instr)) { \
            ctx->ip += extraDataSize(instrType1(instr)); \
        } \
        opcode = instrOpcode(instr); \
        DO_TRACE( \
            if (shlen(ctx->opcodesToBeTraced) > 0) { \
                if (shgeti(ctx->opcodesToBeTraced, "*") != -1 || shgeti(ctx->opcodesToBeTraced, ctx->currentCodeName) != -1) { \
                    char opStr[32], opdStr[256] = "", cmtStr[128] = ""; \
                    formatInstruction(ctx, ctx->bytecodeBase, instrAddr, instr, extraData, opStr, sizeof(opStr), opdStr, sizeof(opdStr), cmtStr, sizeof(cmtStr)); \
                    fprintf(stderr, "VM: [%s] @%04X [0x%08X] %s %s [stack=%d]\n", ctx->currentCodeName, instrAddr, instr, opStr, opdStr, ctx->stack.top); \
                } \
            } \
        ); \
    ...
    // Запуск первой инструкции
    DISPATCH();

op_push:      handlePush(ctx, instr, extraData); DISPATCH();
op_pushloc:   handlePushLoc(ctx, instr, extraData); DISPATCH();
op_pushglb:   handlePushGlb(ctx, instr, extraData); DISPATCH();
op_pushbltn:  handlePushBltn(ctx, instr, extraData); DISPATCH();
op_pushi:     handlePushI(ctx, instr); DISPATCH();

op_pop:       handlePop(ctx, instr, extraData); DISPATCH();
op_popz:      handlePopz(ctx); DISPATCH();

op_add:       handleAdd(ctx); DISPATCH();
op_sub:       handleSub(ctx); DISPATCH();
op_mul:       handleMul(ctx); DISPATCH();
op_div:       handleDiv(ctx); DISPATCH();
op_rem:       handleRem(ctx); DISPATCH();
op_mod:       handleMod(ctx); DISPATCH();

op_and:       handleAnd(ctx); DISPATCH();
op_or:        handleOr(ctx);  DISPATCH();
op_xor:       handleXor(ctx); DISPATCH();
op_shl:       handleShl(ctx); DISPATCH();
op_shr:       handleShr(ctx); DISPATCH();

op_neg:       handleNeg(ctx); DISPATCH();
op_not:       handleNot(ctx, instr); DISPATCH();

op_conv:      handleConv(ctx, instr); DISPATCH();
op_cmp:       handleCmp(ctx, instr); DISPATCH();
op_dup:       handleDup(ctx, instr); DISPATCH();

op_b:         handleBranch(ctx, instr, instrAddr); DISPATCH();
op_bt:        handleBranchTrue(ctx, instr, instrAddr); DISPATCH();
op_bf:        handleBranchFalse(ctx, instr, instrAddr); DISPATCH();

op_call:      handleCall(ctx, instr, extraData); DISPATCH();
...
op_pushenv:   handlePushEnv(ctx, instr, instrAddr); DISPATCH();
op_popenv:    handlePopEnv(ctx, instr, instrAddr); DISPATCH();
